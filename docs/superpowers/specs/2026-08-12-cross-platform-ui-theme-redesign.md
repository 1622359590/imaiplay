...
### 方案 A：各端独立视觉、共享主题契约（采用）

管理后台采用紧凑的数据管理界面，PC 学员端采用宽松的桌面学习界面，H5 采用移动优先界面。三端共享颜色语义、品牌字段、对比度算法和换肤行为，但不强行共享具体组件或 CSS。
...
保留提交 `4636180` 的现代 SaaS 改版作为基线。后续只处理跨端主题契约对齐、发现的可访问性问题和共享默认值漂移，不重新设计已完成页面。
