...
func (service *TenantThemeService) Update(ctx context.Context, update ThemeUpdate) (*domain.Tenant, error) {
	...
	update.LogoURL = strings.TrimSpace(update.LogoURL)
	update.WelcomeText = strings.TrimSpace(update.WelcomeText)
	update.BrowserTitle = strings.TrimSpace(update.BrowserTitle)
	update.BrandName = strings.TrimSpace(update.BrandName)
	if len(update.LogoURL) > 500 || len(update.WelcomeText) > 255 || len(update.BrowserTitle) > 255 || len([]rune(update.BrandName)) > 50 {
	...
	tenant.WelcomeText = update.WelcomeText
	tenant.BrowserTitle = update.BrowserTitle
	tenant.BrandName = update.BrandName
