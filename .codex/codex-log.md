...
## 2026-07-31 租戶默認門戶與統一登錄

### 租戶默認門戶與統一登錄

- 任務執行摘要：完成默認租戶門戶、統一登錄、多企業選擇和可選自定義域名。
- 關鍵修改：新增 Portal Resolver、一次性選擇憑證、租戶一致性校驗與三端會話隔離。
- 評審反饋：平台主域名不得作為租戶域名；密碼驗證成功前不得返回企業信息；未知 Host 不得覆蓋顯式租戶 Header。
- 下一步建議：部署後冒煙驗證三種入口並觀察舊鏈接遷移情況，再評估全局賬號模型。
