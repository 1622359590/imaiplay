...
- 今日统计使用 UTC 自然日边界，Repository 接收起止时间以便测试。
- 完成率为全部课时完成的有效报名学员数除以有效报名学员数。
- 看板仅限 `tenant_admin` / `instructor`，所有聚合查询按 `tenant_id` 隔离。


### SaaS 自助开通

- 客戶可通過 `/api/v1/tenants/register` 完全自助開通租戶，無需 superadmin 介入。
- 開通者自動成為 `tenant_admin`，同一郵箱可在不同租戶註冊，但同一租戶內唯一。
- 租戶代碼由系統自動生成（基於組織名稱或隨機字符串），創建後不可修改。
- 新租戶自動初始化演示數據：1 個示例課程、2 個示例學員、1 個示例講師、示例資源。
- 註冊成功後自動簽發 JWT，前端直接進入管理後台。
- 註冊租戶代碼使用 ASCII slug；中文或空結果回退為 `t-` 隨機後綴，碰撞時追加數字並保持不超過 32 字符。
- 演示資料使用固定課程標題、示例郵箱和資源名稱作為清除標記，清除操作在單一資料庫事務內完成。
