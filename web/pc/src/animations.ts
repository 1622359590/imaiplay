...
export function initScrollReveal() {
  ...
  document.querySelectorAll('.stagger-group').forEach((group) => Array.from(group.children).forEach((child, index) => {
    (child as HTMLElement).style.transitionDelay = reduced ? '0ms' : `${index * 80}ms`;
  }));
  document.querySelectorAll('.reveal').forEach((element) => {
    if (!observedReveal.has(element)) { observedReveal.add(element); revealObserver.observe(element); }
  });
}
...
export function initNavbarScroll() {
  document.querySelectorAll('.app-header, .top-header').forEach((header) => {
