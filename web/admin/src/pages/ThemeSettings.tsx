...
export default function ThemeSettings() {
  ...
  useEffect(() => { void themeApi.get().then(({ data }) => form.setFieldsValue(data)) }, [form])
  const save = async () => { const values = await form.validateFields(); setLoading(true); try { await themeApi.update({ ...values, primary_color: typeof values.primary_color === 'string' ? values.primary_color : '#4F46E5' }); window.dispatchEvent(new Event('tenant-theme-changed')); message.success('主题设置已保存') } finally { setLoading(false) } }
  return <>
    ...
    <Card style={{ maxWidth: 680 }}>
      <Form form={form} layout="vertical" initialValues={{ primary_color: '#4F46E5' }}>
        <Form.Item label="品牌主色" name="primary_color" getValueFromEvent={(_, hex) => hex} rules={[{ pattern: /^#[0-9a-fA-F]{6}$/, message: '请输入六位十六进制颜色，例如 #4F46E5' }]}>
        ...
        <Space><Button type="primary" loading={loading} onClick={() => void save()}>保存主题</Button><Button onClick={() => form.resetFields()}>重置</Button></Space>
      </Form>
    </Card>
  </>
}
